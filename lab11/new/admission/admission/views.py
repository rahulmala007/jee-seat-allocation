...
def updateProfile(request):
	try:
		...
		return render(request,"updateProfile.html",{'name':C.user_name,'password':C.password,'question':C.question,'answer':C.answer,'dob':C.dob,'currentpre':pre})					
	except:
...
def securitycheck(request):
	try:
		...
		if C.answer==answer:
			...
			return HttpResponse("Anser Change Successfully. Go back to Login page to log In.")
		else:
			del request.session['id1']
