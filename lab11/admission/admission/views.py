...
def updatecl(request):
	try:
		C=request.session['candidate_id']
		if C=='Superuser':
			updateCollegeList()
...
def updateRankList():
	try:
		...
		for row in reader:
			if rownum!=0:
				Candidate.objects.create(user_name=row[0],answer=row[12],password=row[12],roll_no=row[0],rankGE=row[3],rankOBC=row[4],rankSC=row[5],rankST=row[6],rankGEPD=row[8],rankOBCPD=row[9],rankSCPD=row[10],rankSTPD=row[11],dob=row[12]) 
			rownum+=1
		...
		return (HttpResponse("Unable to Update.Error Occured"))
...
def showrl(request):
	try:
		...
		return HttpResponse("Cannot Show Rank List.Error Occured")
def showcl(request):
	try:
		...
		return (HttpResponse("Cannot Show College list.Error Occured"))
...
def updateProfile(request):
	try:
		...
		return render(request,"updateProfile.html",{'roll_no':C.roll_no,'category':C.category,'rank':C.rankGE,'pd_status':C.pdStatus,'name':C.user_name,'password':C.password,'question':C.question,'answer':C.answer,'dob':C.dob,'currentpre':pre})					
	except:
		return HttpResponse("Cannot Update Profile.Error Occured.Possibly wrong input.")
...
def securitycheck(request):
	try:
		...
		if C.answer==answer:
			...
			return HttpResponse("Password Change Successfully. Go back to Login page to log In.")
		else:
			del request.session['id1']
			return HttpResponse("Answer Not matched.Go back to Login page to try again.")
	except:
		return HttpResponse("Error Occured.Go back to Login page to try again.")		
...
def givepref(request):
	try:	
		...
		stats=get_all_logged_in_users()
		return render(request,'BranchPref.html',{'user_name':C.user_name,'clist':clist,'users':stats[0],'avg':stats[1],'std':stats[2],})	
	except:
		return HttpResponseRedirect('/updateProfile/')
...
def updatebranches(request):
	try:
		...
		return HttpResponse('Unable to Update branches.Try again')
...
def deletebranches(request):
	try:
		...
		return HttpResponse('Unable to delete branches.Try again')
